...
def flash_firmware():
    ''' Flashes test firmware. Assumes atmel-dfu bootloader is already flashed to board. '''
    global mcu_reset
    ...
    if (retval != 0) and (retval != 256): # For some reason the reset retval is 256 on success in some cases
        send_string(33) # Error
        print(f'{Fore.RED}############################')
